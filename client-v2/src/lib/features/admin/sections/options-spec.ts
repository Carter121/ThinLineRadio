...
export const OPTION_PANELS: OptionPanelSpec[] = [
	{
		...
		fields: [
			...
			{ key: 'defaultSystemDelay', label: 'Default System Delay', type: 'number', min: 0, step: 1, caption: 'Minutes to delay calls for new systems.' },
	...
	},
	{
		...
		fields: [
			...
			{ key: 'emailLogoBorderRadius', label: 'Logo Border Radius', type: 'text', placeholder: '0px', caption: 'CSS radius applied to the email logo (e.g. 8px, 50%).', showIf: (v) => !!v['emailLogoFilename'] }
		]
	},
	{
		id: 'alerts',
		label: 'Alerts & Health',
		fields: [
			{ key: 'alertRetentionDays', label: 'Alert Retention Days', type: 'number', min: 0, step: 1, placeholder: '30', caption: 'Days to keep alert history; 0 keeps it forever.' },
			{ key: 'systemHealthAlertsEnabled', label: 'System Health Alerts', type: 'toggle', caption: 'Master switch for health monitoring alerts.' },
			{ key: 'transcriptionFailureAlertsEnabled', label: 'Transcription Failure Alerts', type: 'toggle', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') },
			{ key: 'transcriptionFailureThreshold', label: 'Failure Threshold', type: 'number', min: 1, placeholder: '5', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') && truthy(v, 'transcriptionFailureAlertsEnabled') },
			{ key: 'transcriptionFailureTimeWindow', label: 'Failure Time Window', type: 'number', min: 1, max: 168, placeholder: '24', caption: 'Hours.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') && truthy(v, 'transcriptionFailureAlertsEnabled') },
			{ key: 'transcriptionFailureRepeatMinutes', label: 'Failure Repeat Interval', type: 'number', min: 15, step: 15, placeholder: '60', caption: 'Minutes between repeated alerts.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') && truthy(v, 'transcriptionFailureAlertsEnabled') },
			{ key: 'toneDetectionAlertsEnabled', label: 'Tone Detection Alerts', type: 'toggle', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') },
			{ key: 'toneDetectionIssueThreshold', label: 'Tone Issue Threshold', type: 'number', min: 1, placeholder: '10', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') && truthy(v, 'toneDetectionAlertsEnabled') },
			{ key: 'toneDetectionTimeWindow', label: 'Tone Time Window', type: 'number', min: 1, max: 168, placeholder: '24', caption: 'Hours.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') && truthy(v, 'toneDetectionAlertsEnabled') },
			{ key: 'toneDetectionRepeatMinutes', label: 'Tone Repeat Interval', type: 'number', min: 15, step: 15, placeholder: '60', caption: 'Minutes between repeated alerts.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') && truthy(v, 'toneDetectionAlertsEnabled') },
			...
			{ key: 'autoLearnToneSetConfig.bToneMaxDuration', label: 'B-Tone Max Duration', type: 'number', min: 0.1, step: 0.1, caption: 'Seconds.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') },
			{ key: 'autoLearnToneSetConfig.longToneMinDuration', label: 'Long Tone Min Duration', type: 'number', min: 1, step: 0.5, caption: 'Seconds.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') },
			{ key: 'autoLearnToneSetConfig.longToneMaxDuration', label: 'Long Tone Max Duration', type: 'number', min: 0, step: 0.5, caption: 'Seconds; 0 means no long tone.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') },
			{ key: 'autoLearnToneSetConfig.callsRequired', label: 'Auto-Learn Calls Required', type: 'number', min: 2, step: 1, caption: 'Matching calls needed before a tone set is learned.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') },
			{ key: 'autoLearnToneSetConfig.frequencyToleranceHz', label: 'Frequency Tolerance', type: 'number', min: 1, step: 1, caption: 'Hz.', showIf: (v) => truthy(v, 'systemHealthAlertsEnabled') }
		]
	},
	{
		id: 'audio',
		...
		fields: [
			...
			{ key: 'disableDuplicateDetection', label: 'Disable Duplicate Detection', type: 'toggle', caption: 'Skip filtering of duplicate call uploads.' },
			{ key: 'duplicateTimestampWindow', label: 'Duplicate Timestamp Window', type: 'number', min: 100, max: 30000, step: 100, placeholder: '800', caption: 'Milliseconds.', showIf: (v) => !truthy(v, 'disableDuplicateDetection') },
			{ key: 'duplicateDetectionTimeFrame', label: 'Duplicate Cache Retention', type: 'number', min: 1000, step: 1000, placeholder: '30000', caption: 'Milliseconds.', showIf: (v) => !truthy(v, 'disableDuplicateDetection') },
			{ key: 'audioEncryptionEnabled', label: 'Audio Encryption', type: 'toggle', caption: 'Requires a relay server API key.', showIf: (v) => !!v['relayServerAPIKey'] },
			{ key: 'maxDownloadsPerWindow', label: 'Max Downloads Per Window', type: 'number', min: 0, placeholder: '100', caption: 'Download rate limit; 0 disables rate limiting.' },
			{ key: 'downloadWindowMinutes', label: 'Download Window', type: 'number', min: 1, max: 60, placeholder: '60', caption: 'Minutes.', showIf: (v) => Number(v['maxDownloadsPerWindow']) > 0 }
		]
	},
	{
		id: 'email',
		label: 'Email',
		fields: [
			{ key: 'emailServiceEnabled', label: 'Email Service', type: 'toggle', caption: 'Master switch for outgoing email (verification, alerts, resets).' },
			{
				key: 'emailProvider',
			...
			{ key: 'emailSendGridApiKey', label: 'SendGrid API Key', type: 'text', masked: true, placeholder: 'SG.xxxx', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'sendgrid' },
			{ key: 'emailMailgunApiKey', label: 'Mailgun API Key', type: 'text', masked: true, placeholder: 'key-xxxx', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'mailgun' },
			{ key: 'emailMailgunDomain', label: 'Mailgun Domain', type: 'text', placeholder: 'mg.example.com', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'mailgun' },
			{
			...
			{ key: 'emailSmtpHost', label: 'SMTP Host', type: 'text', placeholder: 'smtp.example.com', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'smtp' },
			{ key: 'emailSmtpPort', label: 'SMTP Port', type: 'number', min: 1, max: 65535, placeholder: '587', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'smtp' },
			{ key: 'emailSmtpUsername', label: 'SMTP Username', type: 'text', placeholder: 'user@example.com', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'smtp' },
			{ key: 'emailSmtpPassword', label: 'SMTP Password', type: 'text', masked: true, showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'smtp' },
			{ key: 'emailSmtpUseTLS', label: 'Use TLS/SSL', type: 'toggle', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'smtp' },
			{ key: 'emailSmtpSkipVerify', label: 'Skip Certificate Verification', type: 'toggle', showIf: (v) => truthy(v, 'emailServiceEnabled') && v['emailProvider'] === 'smtp' },
			{ key: 'emailSmtpFromEmail', label: 'From Email Address', type: 'text', placeholder: 'noreply@example.com', showIf: (v) => truthy(v, 'emailServiceEnabled') },
	...
	},
	{
		...
		fields: [
			...
			{ key: 'turnstileSecretKey', label: 'Turnstile Secret Key', type: 'text', masked: true, placeholder: '0x4xxxx', showIf: (v) => truthy(v, 'turnstileEnabled') }
		]
	}
];
