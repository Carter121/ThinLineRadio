	...
	{#each value as set, index (set.id ?? index)}
		<div class="rounded-md border border-border p-3">
			<div class="flex items-center gap-2">
				<Input
					value={set.label}
					oninput={(e) => update(index, { label: (e.currentTarget as HTMLInputElement).value })}
					placeholder="Tone set label (e.g. Station 4)"
					class="h-8"
				/>
						...
						<div class="grid grid-cols-3 gap-1">
							<div class="grid gap-0.5">
								...
								<Input
									...
									step="0.1"
									value={s?.frequency ?? ''}
									oninput={(e) => updateSpec(index, spec.key, 'frequency', (e.currentTarget as HTMLInputElement).value)}
							...
							<div class="grid gap-0.5">
								...
								<Input
									...
									step="0.01"
									value={s?.minDuration ?? ''}
									oninput={(e) => updateSpec(index, spec.key, 'minDuration', (e.currentTarget as HTMLInputElement).value)}
									class="h-7 px-1.5 text-xs"
								/>
							</div>
							<div class="grid gap-0.5">
								...
								<Input
									...
									step="0.01"
									value={s?.maxDuration ?? ''}
									oninput={(e) => updateSpec(index, spec.key, 'maxDuration', (e.currentTarget as HTMLInputElement).value)}
									class="h-7 px-1.5 text-xs"
								/>
							</div>
						</div>
			...
			<div class="mt-2 grid gap-2 sm:grid-cols-2">
				<div class="grid gap-1">
					...
					<Input
						...
						step="0.01"
						value={set.tolerance ?? ''}
						oninput={(e) => update(index, { tolerance: Number((e.currentTarget as HTMLInputElement).value) || 0 })}
						class="h-8"
					/>
				...
				<div class="grid gap-1">
					...
					{#if set.downstreamEnabled}
						<Input
							value={set.downstreamURL ?? ''}
							oninput={(e) => update(index, { downstreamURL: (e.currentTarget as HTMLInputElement).value })}
							placeholder="https://host/api/tone-alert"
							class="h-8"
						/>
						<Input
							value={set.downstreamAPIKey ?? ''}
							oninput={(e) => update(index, { downstreamAPIKey: (e.currentTarget as HTMLInputElement).value })}
							placeholder="API key"
