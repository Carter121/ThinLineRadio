<script lang="ts">
	...
	import { formatDuration } from '$lib/core/format.ts';
	import { getTlrClient, getAudioCoordinator } from '$lib/core/context.ts';
	...
	import FavoriteStar from '$lib/features/call-history/FavoriteStar.svelte';
	...
	const table = createSvelteTable<EnrichedCallResult>({
		...
		state: {
			...
			get columnVisibility() {
				return !mobileScreen.current ? { systemLabel: false, source: false } : ({} as Record<string, boolean>);
			}
		}
	});
