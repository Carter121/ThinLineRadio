<script lang="ts">
	...
	import { formatRelativeTime, formatAbsoluteTime, formatDuration, formatFrequency } from '$lib/core/format.ts';
	import type { AudioPlayerState } from './AudioPlayerState.svelte.ts';
	import type { TlrConfig } from '$lib/core/types.ts';

	let { player, config, now }: { player: AudioPlayerState; config: TlrConfig | null; now: SvelteDate } = $props();
		...
		{:else}
			<div class="space-y-2.5">
				...
				<div class="space-y-1">
					...
					<div class="h-57 overflow-y-auto rounded-md border border-border/40">
