...
func (api *Api) AlertsHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		query := fmt.Sprintf(`SELECT a."alertId", a."callId", a."systemId", a."talkgroupId", a."alertType", a."toneDetected", a."toneSetId", a."keywordsMatched", a."transcriptSnippet", a."createdAt", s."label" as "systemLabel", s."systemRef" as "systemRef", t."label" as "talkgroupLabel", t."name" as "talkgroupName", c."transcript" as "callTranscript", c."transcriptionStatus" as "callTranscriptionStatus", c."toneSequence" as "callToneSequence", c."timestamp" as "callTimestamp", c."alertSummary" as "callAlertSummary", c."parsedAddress" as "callParsedAddress", ic."incidentId" as "incidentId", i."fireTier" as "fireTier" FROM "alerts" a LEFT JOIN "systems" s ON s."systemId" = a."systemId" LEFT JOIN "talkgroups" t ON t."talkgroupId" = a."talkgroupId" LEFT JOIN "calls" c ON c."callId" = a."callId" LEFT JOIN "incidentCalls" ic ON ic."callId" = a."callId" LEFT JOIN "incidents" i ON i."incidentId" = ic."incidentId" %s ORDER BY a."createdAt" DESC LIMIT %d`, whereClause, maxAlerts)
		...
		for rows.Next() {
			...
			if incidentFireTier.Valid && incidentFireTier.String != "" {
				alertMap["fireTier"] = incidentFireTier.String
			}
